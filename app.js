...
function playerPortrait(p, size = 'small') {
  const initials = String(p.nome || '?')
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
    .map((part) => part[0])
    .join('')
    .toUpperCase();
  const src = `./dati/campioncini/${encodeURIComponent(String(p.id))}.png`;
  return `<span class="portrait portrait-${esc(size)}" aria-hidden="true"><img src="${esc(src)}" alt="" loading="lazy" decoding="async" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span class="portrait-fallback" hidden>${esc(initials)}</span></span>`;
}
...
function renderMiniPlayer(p, market) {
  ...
  return `<button class="player-card" data-action="detail" data-id="${esc(p.id)}" style="text-align:left"><span class="player-leading">${playerPortrait(p, 'small')}${roleBadge(p)}</span><span class="player-main"><span class="player-name">${esc(p.nome)}</span><span class="player-meta"><span>${esc(p.squadra)}</span><span>${esc(p.verdetto || 'Analisi')}</span></span></span><span class="player-numbers"><strong>${money(max)}</strong><small>tetto</small></span><span class="button-secondary small">Apri</span></button>`;
}
...
function renderAuctionPlayer(p, market) {
  ...
  return `<article class="player-card ${sold ? 'sold' : ''}"><button class="player-leading" aria-label="Apri ${esc(p.nome)}" data-action="detail" data-id="${esc(p.id)}">${playerPortrait(p, 'small')}${roleBadge(p)}</button><button class="player-main" style="border:0;background:transparent;color:inherit;text-align:left;padding:0" data-action="detail" data-id="${esc(p.id)}"><span class="player-name">${esc(p.nome)}${sold ? ` <span class="faint">· venduto a ${esc(state.asta.squadre[sold.squadra] || 'squadra')}</span>` : ''}</span><span class="player-meta"><span>${esc(p.squadra)}</span><span>${esc(p.verdetto || 'Analisi')}</span>${adv.titolare_previsto ? '<span class="green">probabile titolare</span>' : ''}</span></button><span class="player-numbers"><strong>${sold ? money(sold.prezzo) : money(max)}</strong><small>${sold ? 'pagato' : 'tetto'}</small></span><button class="button-secondary small" data-action="detail" data-id="${esc(p.id)}">Apri</button><span class="risk ${riskClass}">${esc(p.rischio || 'Ignoto')}</span></article>`;
}
...
function renderRankList(list, market, value) {
  ...
  return `<div class="rank-list">${list.map((p, index) => `<button class="rank-row" style="border:0;background:transparent;color:inherit;width:100%;text-align:left" data-action="detail" data-id="${esc(p.id)}"><span class="rank-number">${index + 1}</span><span class="rank-player">${playerPortrait(p, 'tiny')}<span><strong>${esc(p.nome)}</strong><small>${esc(p.squadra)} · ${esc(p.verdetto || 'analisi')}</small></span></span><span class="rank-value">${esc(value(p))}</span></button>`).join('')}</div>`;
}
...
function renderFormationPitch(formation) {
  return `<div class="pitch">${['A', 'C', 'D', 'P'].map((role) => `<div class="pitch-row">${(formation.selected[role] || []).map((entry) => `<div class="pitch-player">${playerPortrait(entry.p, 'pitch')}<span>${esc(entry.p.nome)}</span></div>`).join('')}</div>`).join('')}</div>`;
}
